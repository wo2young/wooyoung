...
//			TODO 코드 페이지
			...
			case "item.list": {
			    ...
			    List<ItemMasterDTO> items =
			        (kind == null || "ALL".equalsIgnoreCase(kind))
			...
			case "item.update": {
			    ...
			    redirect(req, resp, "/master?act=item.edit&itemId=" + d.getItem_id() + "&msg=" + (ok ? "upd_ok" : "upd_fail"));
			    return;
			}
			...
			case "routing.update": {
			    ...
			    redirect(req, resp, "/master?act=routing.edit&routing_id=" + d.getRouting_id() + "&msg=" + (ok ? "upd_ok" : "upd_fail"));
			    return;
			}
