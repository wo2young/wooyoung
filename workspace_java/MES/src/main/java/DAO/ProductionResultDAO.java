...
public class ProductionResultDAO {
	...
	public ProductionResultDAO() {
		try {
			...
			DataSource dataFactory2 = (DataSource) ctx.lookup("java:comp/env/jdbc/oracle"); // 유지
			this.ds = dataFactory;
		} catch (Exception e) {
	...
	private int bindFilters(PreparedStatement ps, java.sql.Date from, java.sql.Date to, Integer orderId, int startIdx)
			throws SQLException {
		int idx = startIdx;
		if (from != null)
			ps.setDate(idx++, from);
	...
	/* ===== 실적 등록 ===== */
	public int insertResult(int orderId, int workerId, BigDecimal goodQty, BigDecimal failQty, String processName,
			String processCode, String lotNo, Timestamp workDate) throws SQLException {

		final String sql = "INSERT INTO PRODUCTION_RESULT \n" + " (RESULT_ID, ORDER_ID, WORKER_ID, \n"
	...
	public int updateOrderStatus(int orderId, String status) throws SQLException {
		final String sql = "UPDATE PRODUCTION_ORDER SET STATUS=? WHERE ORDER_ID=?";
		try (Connection conn = ds.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
	...
	/* ===== 상세 실적 조회 ===== */
	public List<Map<String, Object>> selectResults(java.sql.Date from, java.sql.Date to, Integer orderId, int page,
			int size) {
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT * FROM ( \n").append(
				"  SELECT inner_q.*, ROW_NUMBER() OVER(ORDER BY inner_q.work_time DESC, inner_q.result_id DESC) rn \n")
...
// ★★★ 이 부분을 지시일(CREATED_AT)을 기준으로 변경 ★★★
		if (from != null)
			sb.append("      AND TRUNC(o.CREATED_AT) >= ? \n");
		if (to != null)
			sb.append("      AND TRUNC(o.CREATED_AT) <= ? \n"); // 당일 포함
		if (orderId != null && orderId > 0)
	...
	/* ===== 카운트 ===== */
	public int countResults(Date from, Date to, Integer orderId) {
		StringBuilder sb = new StringBuilder();
	...
	/* ===== 달성률 요약 ===== */
	public List<Map<String, Object>> selectSummary(Date from, Date to, Integer orderId) {
		StringBuilder sb = new StringBuilder();
	...
	public List<Map<String, Object>> getOpenOrders(String siteCode) {
		String statusClause = "UPPER(TRIM(po.STATUS)) IN ('PLANNED','IN_PROGRESS','RUNNING','진행중','계획')";
